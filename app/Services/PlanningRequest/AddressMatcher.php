...
class AddressMatcher
{
    ...
    /**
     * Срезает ВЕДУЩИЙ тип «улица»/«ул.» для хранения/отображения, сохраняя
     * регистр остального названия: «улица Люблинская» → «Люблинская»,
     * «ул. Ленина» → «Ленина».
     *
     * Намеренно срезаем ТОЛЬКО «улица/ул.» — именно их дублирует жёсткий
     * префикс «ул.» в интерфейсе. Проспект/бульвар/шоссе/переулок и т.п. НЕ
     * трогаем: иначе «проспект Мира» превратился бы в «ул. Мира» (неверный
     * тип). Тип в середине/конце («Скорняжный переулок») тоже не трогаем.
     * Если после среза пусто — возвращаем исходную строку.
     *
     * Используется импортом и командой addresses:strip-street-type.
     */
    public static function stripLeadingStreetType(string $street): string
    {
        $original = trim($street);

        $stripped = preg_replace('/^\s*(?:улица|ул\.)\s+/iu', '', $original);
