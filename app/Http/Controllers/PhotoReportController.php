...
class PhotoReportController extends Controller
{
    ...
    private function sanitizeFolderName($name)
    {
        ...
        // "*" используется монтажниками как метка "важно" — заменяем на полноширинную
        // звёздочку "＊" (другой символ юникода, не запрещён в именах файлов Windows),
        // чтобы метка сохранялась визуально при распаковке архива на Windows.
        $name = str_replace('*', '＊', $name);
        // Заменяем остальные недопустимые символы на подчеркивание (используем ~ как разделитель regex)
