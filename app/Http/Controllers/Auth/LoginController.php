...
class LoginController extends Controller
{
    public function showLoginForm()
    {
        try {
            ...
            Log::error('Error in Auth\LoginController@showLoginForm: '.$e->getMessage());

            return response('Произошла ошибка при загрузке страницы входа', 500);
        }
    }
    ...
    public function login(Request $request)
    {
        try {
            ...
            Log::error('Error in Auth\LoginController@login: '.$e->getMessage());

            return back()
                ->withInput($request->only('login', 'remember'))
                ->with('error', 'Произошла ошибка во время входа. Пожалуйста, попробуйте еще раз.');
        }
    }
}
