...
* **Хранение задач**: таблица `ai_tasks` в БД `lan_install`. Просмотр последней: `sudo psql -U postgres -d lan_install -c "SELECT * FROM ai_tasks ORDER BY id DESC LIMIT 1;"`

## ⚠️ Критерий завершения задачи
Задача считается завершённой ТОЛЬКО после всех шагов: 1) программные тесты (PHPUnit), 2) ручная проверка пользователем, 3) коммит, 4) деплой через `./git-deploy`. Запрещено вызывать `./log-task finish` до выполнения всех шагов.
