...
**ИНЦИДЕНТ 2026-07-26/27 — прод три дня работал на СТАРОМ коде (от 01.07), РЕШЕНО:**
- Симптом: заказчик сообщил, что «перестали работать» выбор цвета текста, скрытие бригады и
      комментарий закрытия. Все три — фичи из коммитов 14–23 июля.
- Причина: код на `.67` заехал rsync-ом в Фазе 2 (**12 июля**), после чего 12 коммитов
      деплоились только на `.10`. В чек-листе cutover **не было шага синхронизации кода** —
      синкали БД и фото, а код нет. Плюс на `.67` **не был установлен git**, поэтому `git pull`
      туда не делался никогда. Итог: с полуночи 24.07 прод крутился на `bc8b76c` (01.07),
      отставание 12 коммитов / 29 файлов / 1836 строк.
- Почему не заметили: после cutover проверяли **доступность** доменов (200 OK), а не версию кода.
- Фикс (26.07 22:34): `apt install git` на `.67`; код доставлен с `.10` через `git bundle`
      (`bc8b76c..main`, 112 КБ) → `git fetch` + `merge --ff-only` → `3fb55cb`; `config/cache/view/route:clear`
      + `systemctl reload php8.2-fpm` (сброс opcache). Файлы сверены с `.10` по md5 — идентичны.
- Добивка данных (27.07): за «слепой» период 24–26.07 у 24 закрытых заявок не проставился
      `is_closing` (кода не было) → комментарий закрытия не закреплялся. Проставлен задним числом
      строго по совпадению `comment.created_at == request.closed_at` (±1 сек) и только там, где у
      заявки не было ни одного флага: `UPDATE 24`, итого 77 → 101. Бэкап снят до правки.
- **Урок в чек-лист cutover:** синхронизировать КОД так же, как БД и фото, и после переключения
      **сверять коммит на новом сервере** (`git log -1`), а не только доступность сайтов.

- [ ] **git-deploy всё ещё нацелен на `.10`** — при следующем деплое код уедет на старый сервер,
      а прод останется без изменений (повтор этого же инцидента). Перенацелить на `.67`.
      ⚠️ Приоритет: сделать ДО следующего деплоя.
- [ ] `.67` не умеет ходить в GitHub: у www-data нет deploy-ключа (на `.10` GitHub-доступ у root
      через `/root/.ssh/git_lan_install`). Сейчас обновление возможно только через `git bundle`
      с `.10`. Завести отдельный deploy-key для `.67`.
- [ ] `git config --system --add safe.directory /var/www/lan-install` на `.67` — иначе git от
      имени не-www-data ругается `dubious ownership`.
