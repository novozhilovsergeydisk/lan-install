...
* **Деплой:** Перед отправкой изменений **обязательно** проанализируй вывод `git diff` на предмет регрессий, ошибок логики и наличия секретов. Опиши результаты анализа. Если всё в порядке — сразу вызывай скрипт `./git-deploy "комментарий"`, который автоматизирует фиксацию и обновление кода на сервере. Не задавай лишних вопросов — полагайся на системный промпт подтверждения инструмента.
