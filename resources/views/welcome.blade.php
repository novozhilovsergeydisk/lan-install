...
@endif
			...
			<form action="{{ route('logout') }}" method="POST" class="mb-0">
    				@csrf
    				<button type="submit" class="btn btn-outline-danger btn-sm mr-5 px-3" style="margin-right: 2rem;"><i class="bi bi-box-arrow-right me-1"></i> Выход</button>
			</form>
        ...
        <ul>
            ...
            <li><strong>Дата регистрации:</strong> {{ session('user_data')['created_at'] }}</li>
        </ul>
    @endif
@if(session('migrations'))
        ...
        <ul>
            @foreach(session('migrations') as $migration)
                <li>
                    <strong>{{ $migration->migration }}</strong> (ID: {{ $migration->id }}, Batch: {{ $migration->batch }})
                </li>
            @endforeach
        </ul>
    @endif
    ...
    <ul class="list-group">
        @foreach(session('clients') as $client)
            <li class="list-group-item d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-primary">{{ $client->phone ?? 'Нет телефона' }}</span>
            </li>
        @endforeach
    </ul>
@else
