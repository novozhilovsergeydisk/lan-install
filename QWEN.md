...
* **Безопасность:** Никогда не хардкодить пароли, токены и ключи API в исходном коде или документации (QWEN.md). Всегда выносить чувствительные данные в переменные окружения (`.env`) или локальные файлы конфигурации (`telegram.conf`), которые добавлены в `.gitignore`.
...
* **Деплой:** Перед отправкой изменений **обязательно** проанализируй вывод `git diff` на предмет регрессий, ошибок логики и наличия секретов. Опиши результаты анализа. Выводи на экран — сразу вызывай скрипт `./git-deploy "комментарий"`, который автоматизирует фиксацию и обновление кода на сервере. Не задавай лишних вопросов — полагайся на системный промпт подтверждения инструмента.
...
* **Модели:** Не используюся в проекте, применятся нативные sql запросы и фасад DB для доступа к объектам базы данных.
