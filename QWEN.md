...
Для сервера (Linux):
```bash
sudo -u postgres psql -d lan_install -c "\d"
```
...
* Файл `public/js/init.js` использовать для инициализации обработчиков и другого кода после полной загрузки DOM.
...
* **Деплой:** Перед деплоем всегда проводить следующие действия: проанализируй внимательно вывод команды `git status` и `git diff` и оцени, могли ли внесённые изменения нарушить корректную работу существующего кода, нет ли в изменениях чувствительных данных. Обрати внимание на потенциальные регрессии, изменения в логике, сигнатурах функций, обработке ошибок или зависимостях и опиши мне ответы по итогам этих изменений в проекте - это нужно для комментария коммита в соответствии с Conventional Commits, отвечай на русском языке. Также сформулируй для заказчика ответ по текущим изменениям в проекте в виде одного понятного предложения (например, "Я добавил проверку лишних колонок при загрузке заявок и перенес кнопку скачивания шаблона для удобства"). Выводи эту информацию на экран и жди подтверждения, а если все в порядке и получена команда, то делай деплой через скрипт `deploy` (или `./git-deploy "комментарий"`).
...
## Логирование
*   **Важно:** Основные логи приложения на сервере находятся в файле `storage/logs/local.log`. Всегда проверяй этот файл при возникновении ошибок "Ошибка сервера" или "Network Error".
